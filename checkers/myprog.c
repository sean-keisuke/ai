...
void PrintBoard(char board[8][8])
{
    ...
    for(y=0; y<8; y++)
    {
        for(x=0; x<8; x++)
        {
             if(x%2 != y%2)
             {
                 if(king(board[y][x]))
                 {
                     if(board[y][x] & White) fprintf(stderr,"B");
    ...
    fprintf(stderr, "EVALUATE BOARD VAL: %f\n\n", evalBoard(board));
}
...
double evalBoard(char board[8][8])
{
    ...
    else return sumOfReds-sumOfWhites;

/*
    if(me==2)
        return sumOfReds-sumOfWhites;
    else return sumOfWhites-sumOfReds;
*/
...
{
    //fprintf(stderr, "%d", state->player);
    //double v = MaxVal(state, alpha, beta, depth);
    double v = MinVal(state, alpha, beta, depth);
    
    //fprintf(stderr, "V IS %f\n", v);
    return v;
}
...
double MaxVal(struct State *state, double alpha, double beta, int depth)
{
    ...
    if(depth <= 0)
    {
        //fprintf(stderr, "MAXDEPTHREACHED\n");
    ...
    for(x = 0; x < state->numLegalMoves; x++)
    {
        ...
        if(state->player==1) temp.player=2; //check this later
            else temp.player=1;
        
        memcpy(&temp, state, sizeof(temp)); 

        PerformMove(temp.board, temp.movelist[x], MoveLength(temp.movelist[x])); //perform the move

        v = MAX(v,MinVal(&temp,alpha,beta,depth-1));
...
double MinVal(struct State *state, double alpha, double beta, int depth)
{
    ...
    for(x = 0; x < state->numLegalMoves; x++)
    {
        ...
        memcpy(&temp,state, sizeof(temp));
         ...
         if(state->player==1) temp.player=2;
            else temp.player=1;


        PerformMove(temp.board, temp.movelist[x], MoveLength(temp.movelist[x])); //perform the move

        v = MIN(v,MaxVal(&temp,alpha,beta,depth-1));
        if(v <= alpha)
        {
            //fprintf(stderr, "V IS %f\n", v);
            return v;
...
void oldFindBestMove(int player)
{
    int i; 
    struct State state; 
...
void FindBestMove(int player)
{
    ...
    double alpha = -100000;
    double beta = 100000;
    bestindex = rand()%state.numLegalMoves; //when timer expires, at least return something
    double best = alpha;
    ...
    for(x = 0; x < state.numLegalMoves; x++)
    {
        ...
        rval = alphaBetaSearch(&temp, alpha, beta, depth);
        if(rval > alpha)
        {
            alpha = rval;
            //bestmove
...
int testmain(int argc, char *argv[])
{
    ...
    //double val = evalBoard(testBoard);
